...
iso_codes <- countrycode::codelist[, c("un.name.en", "iso3c")]
world_data <- ggplot2::map_data('world') %>% fortify()
...
```{r}
world_data_with_iso3[world_data_with_iso3$region %in% "Sweden", c("region", "iso3c")]
```
```{r}
filtered_data_world <-
  data %>%
...
```{r}
# Merge the world_data_with_iso3 with filtered_data_world
hm_df <- 
  world_data_with_iso3 %>%
